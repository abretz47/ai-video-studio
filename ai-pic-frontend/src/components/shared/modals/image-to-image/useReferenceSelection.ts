...
export function useReferenceSelection({
  ...
  const notes = useMemo(() => {
    ...
    if (referenceSectionsLength > 0 && !allowMultipleRefs) {
      next.unshift(
        "This model does not support multiple reference images: only 1 reference image will be used (other selections will be replaced/ignored)",
      );
    }
    if (
      ...
      maxRefs !== undefined &&
      maxRefs > 1 &&
      !next.some((note) => note.includes(`${maxRefs} reference images`))
    ) {
      next.unshift(
        `This model supports up to ${maxRefs} reference images: if you exceed the limit, the earliest selected reference image will be replaced automatically`,
      );
    }
    return next;
