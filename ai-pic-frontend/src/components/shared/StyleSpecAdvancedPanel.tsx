...
export function StyleSpecAdvancedPanel({
  title = "Advanced Style (only selected dimensions are sent)",
  helperText = "Unselected dimensions are automatically filled in by the backend based on preset / defaults.",
  fields,
  value,
            ...
            {effectiveFields.map((field) => {
              ...
              return (
                <div key={field.key} className="space-y-1">
                  ...
                  >
                    <option value="">(Follow preset/defaults)</option>
                    {options.map((opt) => (
