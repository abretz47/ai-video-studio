...
export function ProductionCanvasBoard() {
  return (
    <OperatorShell
      title={t("canvas.board.title", "Creation Canvas")}
      subtitle={t("canvas.board.subtitle", "Orchestrate scripts, storyboards, image candidates, video candidates, and timelines from existing projects")}
      breadcrumb={[
        t("common.breadcrumb.ipCenter", "IP Center"),
        t("canvas.board.breadcrumb", "Creation Canvas"),
      ]}
      ...
      rightSlot={
        <div className="hidden sm:block">
          ...
          >
            {t("canvas.board.backToStories", "Back to Story Production")}
          </Link>
        </div>
      }
    >
    ...
    <div className="space-y-4">
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_280px]">
        <OperatorPanel className="overflow-hidden">
          <OperatorSectionHeader
            title={t("canvas.board.chainTitle", "Short Drama Production Flow")}
            ...
            action={
              <Link href="/tasks" className={operatorButtonClass("ghost")}>
                {t("canvas.board.viewTasks", "View Tasks")}
              </Link>
            }
          />
          ...
          <div className="flex flex-wrap items-center gap-2 border-b border-gray-200 px-4 py-2">
            ...
            >
              {t("canvas.board.addNote", "Add Note")}
            </button>
            ...
            <button
              type="button"
              aria-label={t("canvas.board.zoomOut", "Zoom out")}
              title={t("canvas.board.zoomOut", "Zoom out")}
            ...
            <button
              type="button"
              aria-label={t("canvas.board.zoomIn", "Zoom in")}
              title={t("canvas.board.zoomIn", "Zoom in")}
              className={operatorButtonClass("secondary", "w-8 px-0")}
              onClick={() => handleZoomButton(1)}
            >
              +
        ...
        <div className="space-y-3">
          ...
          <OperatorPanel className="p-4">
            <div className="text-xs font-semibold text-gray-950">{t("canvas.board.actionsTitle", "Canvas Actions")}</div>
            <div className="mt-2 space-y-1 text-xs leading-5 text-gray-500">
              <p>{t("canvas.board.actionDragNodes", "Drag nodes to reposition the flow.")}</p>
              <p>{t("canvas.board.actionPan", "Drag empty space to move around the canvas.")}</p>
              <p>{t("canvas.board.actionZoom", "Use the mouse wheel or toolbar to zoom.")}</p>
            </div>
          </OperatorPanel>
        </div>
      </div>
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        <OperatorPanel className="p-4">
          <div className="text-xs font-semibold text-gray-950">{t("canvas.board.referencesTitle", "Referenced Objects")}</div>
          <p className="mt-2 text-xs leading-5 text-gray-600">
            IP / Story / Episode / Task / Artifact
          </p>
        </OperatorPanel>
        <OperatorPanel className="p-4">
          <div className="text-xs font-semibold text-gray-950">{t("canvas.board.executionLayer", "Execution Layer")}</div>
          <p className="mt-2 text-xs leading-5 text-gray-600">
            Existing API / Skill Invocation / Artifact Run
          </p>
        </OperatorPanel>
        <OperatorPanel className="p-4">
          <div className="text-xs font-semibold text-gray-950">{t("canvas.board.evidenceOutlet", "Evidence Outputs")}</div>
          <p className="mt-2 text-xs leading-5 text-gray-600">
