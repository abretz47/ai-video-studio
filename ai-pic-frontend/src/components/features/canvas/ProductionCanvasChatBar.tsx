    ...
    <div className="border-b border-gray-200 bg-white px-4 py-3">
      <div className="flex flex-col gap-2 lg:flex-row lg:items-end">
        <label className="min-w-0 flex-1">
          <span className="text-xs font-semibold text-gray-700">{t("canvas.chat.goal", "Production Goal")}</span>
          <textarea
            aria-label={t("canvas.chat.goal", "Production Goal")}
            value={prompt}
            onChange={(event) => onPromptChange(event.target.value)}
            onInput={(event) => onPromptChange(event.currentTarget.value)}
            placeholder={t("canvas.chat.placeholder", "For example: build episode 4 around Lin Meimei, office rom-com tone, and generate the full short-drama pipeline")}
        ...
        <button
          ...
          onClick={onCreate}
        >
          {running ? t("canvas.chat.running", "Running") : t("canvas.chat.create", "Create Full Flow")}
        </button>
      </div>
