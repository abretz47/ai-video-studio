    ...
    <div className="border-b border-gray-200 bg-white px-4 py-3">
      <div className="flex flex-col gap-2 lg:flex-row lg:items-end">
        <label className="min-w-0 flex-1">
          <span className="text-xs font-semibold text-gray-700">Production goal</span>
          <textarea
            aria-label="Production goal"
            value={prompt}
            onChange={(event) => onPromptChange(event.target.value)}
            onInput={(event) => onPromptChange(event.currentTarget.value)}
            placeholder="e.g., Create episode 4 based on Sister Lin, an office rom-com, and generate the full short drama pipeline"
        ...
        <button
          ...
          onClick={onCreate}
        >
          {running ? "Running" : "Create full plan"}
        </button>
      </div>
