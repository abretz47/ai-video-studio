  ...
  return (
    <div>
      <label className="block text-sm font-medium text-gray-700 mb-2">
        {t("stories.characterSelector.label", "Select characters * (at least one)")}
        <span className="text-blue-600 ml-2">
          {t("stories.characterSelector.selectedCount", "Selected: {count}").replace("{count}", String(selectedIds.length))}
        </span>
      </label>

      {virtualIPs.length === 0 ? (
        <OperatorState
          title={t("stories.characterSelector.emptyTitle", "No characters available")}
          detail={t("stories.characterSelector.emptyDetail", "Create a virtual IP character before generating a story.")}
          action={
            <button
              type="button"
            ...
            >
              {t("stories.characterSelector.createIp", "Create virtual IP")}
            </button>
          }
        />
      ) : (
          ...
          {selectedIds.length > 0 && (
            <div className="mt-3 rounded-md border border-blue-200 bg-blue-50 p-2">
              <p className="text-sm text-blue-800">
                {t("stories.characterSelector.selectedNames", "Selected characters")}: {" "}
                {selectedIds
                  .map((id) => {
