  ...
  return (
    <OperatorPanel>
      <OperatorSectionHeader
        ...
        subtitle="Generate a large storyboard grid image per scene, then use Seedance to create a continuous final cut based on that grid."
        action={
          <button
            type="button"
      ...
      {!expanded ? null : (
        <>
          ...
          <label className="mt-3 flex flex-col gap-1 text-xs text-gray-600">
            Environment reference image URLs (optional, separated by commas or new lines; uses scene environment automatically if empty)
            <input
              className="rounded-md border border-gray-300 px-2 py-1.5 text-sm"
              placeholder="https://…"
