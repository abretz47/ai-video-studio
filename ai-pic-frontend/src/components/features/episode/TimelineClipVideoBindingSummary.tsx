  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2">
        ...
        >
          {hasAnyBinding ? "Bindings attached" : "Binding pending"}
        </span>
      </div>
      <div className="mt-2 grid gap-1">
        <BindingLine
          label="Character IP"
          value={characterLabels.length ? characterLabels.join("、") : "Not linked"}
      ...
      <div className="mt-2 leading-4 text-gray-500">
        {hasAnyBinding
          ? "The video task will include the selected IP and environment images above."
          : "Bind character IP and environment images in the clip storyboard above, and the video task will include them."}
      </div>
    </div>
  );
}
