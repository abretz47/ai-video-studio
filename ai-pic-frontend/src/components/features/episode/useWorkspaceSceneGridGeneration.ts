...
export function useWorkspaceSceneGridGeneration({
  ...
  const { tasks, track } = useGenerationTaskTracker<"sheet" | "video">({
    labels: { sheet: "Grid storyboard generation", video: "Grid final cut generation" },
    onCompleted: async () => {
      await refreshGrids();
    },
  ...
  const handleGenerateSheet = useCallback(async () => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        showAlert?.({
          message: `Grid storyboard task submitted #${res.data.task_id}`,
          variant: "success",
        });
      } else {
        showAlert?.({
          message: res.error || "Failed to submit grid storyboard task",
          variant: "error",
        });
      }
    } finally {
