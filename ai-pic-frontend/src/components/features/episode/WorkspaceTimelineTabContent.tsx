...
export function WorkspaceTimelineTabContent({
  ...
  const handleGenerateTimelinePipeline = useCallback(async () => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        showAlert({
          message: `One-click pipeline task submitted (task_id=${res.data.task_id})，The timeline will refresh automatically when completed`,
          variant: "info",
        });
      } else {
        showAlert({
  ...
  if (scripts.length === 0) {
    return (
      <OperatorState
        title="No Script Yet"
        detail="Generate a script before creating a timeline"
      />
    );
  }
