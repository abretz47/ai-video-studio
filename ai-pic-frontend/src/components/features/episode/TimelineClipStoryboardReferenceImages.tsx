  ...
  return (
    <>
      <ReferenceImageSummary
        ...
        emptyText={
          selectedVirtualIpIds.length ? "IP images temporarily unavailable" : "Bind character IP first"
        }
        onReplace={onCharacterImagesReplace}
      />
      <ReferenceImageSummary
        title="Environment Image"
        ariaPrefix="Select Environment Image"
        altPrefix="Environment Image"
        selectedAltPrefix="SelectedEnvironment Image"
        options={environmentImageOptions}
        groups={environmentGroups}
        selectedUrls={selectedEnvironmentUrls}
        emptyText="Environment images temporarily unavailable"
  ...
  return (
    ...
    >
      ...
      {loading ? (
        ...
        <div className="text-[11px] text-red-600">Failed to load images: {error}</div>
      ) : options.length ? (
        <div className="grid gap-2">
          ...
          {selectedOptions.length ? (
            ...
            <div className="text-[11px] text-gray-500">No image selected</div>
          )}
          <ReferenceImagePickerModal
            title={ariaPrefix}
