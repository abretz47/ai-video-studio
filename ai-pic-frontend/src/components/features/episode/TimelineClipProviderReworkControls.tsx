  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!productionReadiness.canGenerateVideo) {
      const message =
        ...
        "Generate the clip storyboard and start/end frames before generating video";
      generationActions.setSubmitError(message);
      onNotify?.(message, "warning");
      return;
    }
    ...
    try {
      ...
      if (!res.success || !res.data) {
        const message = res.error || "Failed to submit the video rework task";
        generationActions.setSubmitError(message);
      ...
      setReason("");
      await onQueued?.();
      onNotify?.(`Video rework task #${res.data.task_id} submitted. Generating…`, "success");
