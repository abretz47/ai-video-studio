...
export function SceneStructurePanel({
  ...
  const addScene = async () => {
    if (!canEdit) {
      showAlert({ message: "This view is read-only and requires admin permissions", variant: "warning" });
      return;
    }
        ...
        {scenes.map((scene) => (
          <div key={scene.id} className="rounded-md border border-gray-200 bg-gray-50 p-3">
            ...
            <div className="mt-3 grid gap-2 md:grid-cols-2">
              <NodeList title={`Beat (${scene.beats.length})`} items={scene.beats.map((beat) => `#${beat.order_index} ${beat.beat_summary || ""}`)} />
