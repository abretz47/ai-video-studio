...
const statItems = (stats: UserStatsResponse) => [
  ...
  { label: "Active Users", value: stats.active_users, href: "/admin/users?status=approved" },
  { label: "Pending Approval", value: stats.pending_approval, href: "/admin/users?status=pending", tone: "amber" as const },
  { label: "Suspended Users", value: stats.suspended_users, href: "/admin/users?status=suspended", tone: "red" as const },
  { label: "Administrators", value: stats.admin_users, href: "/admin/users?role=admin", tone: "blue" as const },
...
export default function AdminStatsPage() {
  ...
  const loadStats = async () => {
    ...
    try {
      ...
      else setError(response.error || "Failed to load statistics");
    } catch (err) {
  ...
  return (
    <OperatorAdminShell title="Statistics" subtitle="Overview of users and approval operations">
      <div className="space-y-4">
        <OperatorPanel>
          <OperatorSectionHeader
            title="User Statistics"
            subtitle="Accounts, approvals, and permission distribution"
            action={
              <button
                type="button"
        ...
        {stats ? (
          <OperatorPanel>
            <OperatorSectionHeader title="Quick Actions" subtitle="Jump from statistics to user operations" />
            <div className="grid gap-3 p-4 md:grid-cols-3">
              <QuickLink href="/admin/users?status=pending" label="Handle Pending Approvals" detail={`${stats.pending_approval} pending`} />
