...
export default function TestAuth() {
  ...
  const testProtectedEndpoint = async (token: string) => {
    try {
      ...
      if (response.ok) {
        ...
        setResult((prev) => `${prev}\nProtected endpoint succeeded and returned ${data.data?.length || 0} items`);
      } else {
  ...
  return (
    <OperatorAuthFrame
      ...
      subtitle="Verify login and protected endpoints"
      switchLabel="Back"
      switchHref="/login"
      switchText="Login Page"
    >
      <OperatorPanel>
        <OperatorSectionHeader title="Diagnostic Actions" subtitle="fetch and API client" />
