...
export function useGenerationTaskTracker<K extends string>({
  ...
  const finishTask = useCallback(
    (
      ...
      if (phase === "completed") {
        onNotifyRef.current?.(`${label}generated successfully and results have been refreshed`, "success");
        ...
        onNotifyRef.current?.(
          `${label}Task #${taskId} wait timed out. Please check the task list later`,
          "warning",
        );
      }
    },
