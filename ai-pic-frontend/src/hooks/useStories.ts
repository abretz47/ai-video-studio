...
export function useStories({ showAlert }: UseStoriesOptions) {
    ...
    try {
      ...
      if (useAsync) {
        ...
        if (response.success) {
          ...
          showAlert({
            message: taskId
              ? `Async task created (ID: ${taskId}). You can track progress on the Tasks page`
              : "Async task created. You can track progress on the Tasks page",
            variant: "info",
      ...
      showAlert({ message: "Story generation failed", variant: "error" });
    } finally {
      setGenerating(false);
    }
  };
  ...
  const handlePreviewPrompt = async () => {
    try {
      ...
      setPromptPreview("Preview error");
    }
  };
