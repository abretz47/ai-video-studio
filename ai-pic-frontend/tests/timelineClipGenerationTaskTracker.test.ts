...
describe("timeline clip generation task tracker", () => {
  ...
  it("polls a queued task to completion and triggers refresh", async () => {
    ...
    await waitFor(() =>
      assert.ok(utils.getByText(/Clip storyboard images generating (Task #88)/)),
    );
    await waitFor(
      () => assert.ok(utils.getByText(/Clip storyboard images generated (Task #88)/)),
      { timeout: 3000 },
    );
    ...
    assert.ok(
      notices.some(
        (notice) =>
          ...
          notice.message.includes("Clip storyboard images generated"),
      ),
    );
  });
