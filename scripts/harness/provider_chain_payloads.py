...
def build_script_prompt(
    ...
    return (
        ...
        "The character tag must be concrete: characters[0].role must combine a personality, "
        "job, and motivation such as 'hot-tempered editor trying to preserve client approval evidence'; do not write only 'protagonist'. "
        ...
        "For the final beat, do not put 'deletion complete', 'already uploaded', 'already fixed', or 'task complete' as the final state; "
        "use 'deleting in progress', 'second countdown', 'unknown operator', or a new active threat instead. "
        ...
        "one marker from: system, permission, client, insider, review, locked, deleted, tampered, modifier, operator. "
        "The opening hook visible_event, action, or dialogue must literally include "
        "one marker from: alert, warning, countdown, zeroed out, deleted, lost, locked, error, failed, crisis, "
        "threat, evidence, truth, reversal, or must. "
        f"{repair_text}"
        ...
        "threat, choice, or result. Do not use generic wording like 'advancing the plot', "
        "'pressure increases', 'chaotic situation', or 'plot twist'. "
        ...
        "Do not use generic speaker names like 'protagonist', 'character', 'male lead', 'female lead', or 'narrator'; "
        ...
        "like 'advancing the plot', 'creating suspense', 'creating conflict', or 'plot twist'. Each beat must include duration_seconds > 0, "
        ...
        "do not write internal states such as 'realizes', 'understands', 'feels', 'inner thoughts', 'fate', or 'relationship changes'. "
        "Every dialogue line must be <= 15 visible characters. "
        "Dialogue must carry story information; do not use filler-only lines like 'okay', 'mm', 'got it', 'yes', or 'how did this happen', "
        ...
        "the countdown reached 100%, or the protagonist says 'too late'; after any partial payoff, "
