...
def test_role_management_ui():
    ...
    try:
        ...
        # 3. Dian Ji Di Yi Ge user details button
        ...
        # 5. Cha Zhao and Dian Ji"management character"button
        role_management_buttons = driver.find_elements(
            By.XPATH, "//button[contains(text(), 'management character')]"
        )
        if len(role_management_buttons) > 0:
            ...
            try:
                WebDriverWait(driver, 10).until(
                    EC.presence_of_element_located(
                        (By.XPATH, "//h3[contains(text(), 'character management')]")
                    )
                )
                ...
                if len(role_radios) >= 3:
                    success_indicators.append("character Xuan Xiang")
                    ...
                    expected_roles = ["Pu general-purpose Hu", "administrator", "Chao Ji administrator"]
                    found_roles = any(
                ...
                if len(permission_lists) > 0:
                    success_indicators.append("permission Shuo Ming")
...
def test_backend_role_api():
    """test after Duan character managementAPI"""
    print("\n🔍 test after Duan character managementAPI")
    ...
    try:
        ...
        if not test_user:
            print("⚠️  Wei Zhao Dao He Shi test user（Fei administrator user）")
            return True
        ...
        if test_response.status_code in [200, 400, 422, 403]:
            print("✅ character managementAPIendpoint exists Qie Ke access")

            if test_response.status_code == 403:
                print("   ⚠️  Dang Qian user permission Bu Zu（Zhe Shi normal An Quan Xian Zhi）")

            return True
        else:
            print(f"❌ character managementAPIendpoint exception: {test_response.status_code}")
...
def main():
    ...
    if overall_success:
        ...
        print("   ✅ user character Xuan Ze (Pu general-purpose Hu/administrator/Chao Ji administrator)")
        print("   ✅ character permission Shuo Ming display")
        print("   ✅ character Bian Geng reason record")
        print("   ✅ permission validate and An Quan Kong Zhi")
        print("   ✅ after DuanAPIintegration")
    else:
        print("\n⚠️ Bu Fen Gong Neng Ce Shi failed，Jian Yi check:")
        if not api_success:
            print("   - after Duan character managementAPI")
