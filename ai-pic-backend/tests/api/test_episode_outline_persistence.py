...
def test_episode_generation_persists_validated_outlines(
        ...
        "episodes": [
            {
                "episode_number": 1,
                "title": "Ep1",
                "logline": "Yi Ju Hua summary",
                "beats": [
                    {
                        "sequence_number": 1,
                        "beat_title": "Kai Duan",
                        "beat_summary": "protagonist Deng Chang",
                    }
                ],
            },
            {"episode_number": 2, "title": "Ep2", "logline": "Di Er Ji Yi Ju Hua"},
        ]
    }
    episodes_payload = {
        "episodes": [
    ...
    assert outline_meta["episodes"][0]["logline"] == "Yi Ju Hua summary"
    ...
    assert len(rows) == 1
    assert rows[0].beat_title == "Kai Duan"
...
def test_episode_generation_falls_back_to_outline_when_content_invalid(
    ...
    story = Story(title="Fallback story", genre="drama")
    db_session.add(story)
    db_session.commit()
    db_session.refresh(story)
    ...
    outlines = {
        "episodes": [
            {"episode_number": 1, "title": "E1", "logline": "Yi Ju Hua1"},
            {"episode_number": 2, "title": "E2", "logline": "Yi Ju Hua2"},
        ]
    }
    ...
    body = resp.json()
    assert len(body) == 2
    assert body[0]["summary"] == "Yi Ju Hua1"
    assert body[1]["summary"] == "Yi Ju Hua2"
