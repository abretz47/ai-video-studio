...
def test_build_script_prompt_accepts_optional_premise() -> None:
    prompt = build_script_prompt("full-30s", "bonus reset to zero，robot Bi Xu Zhao Chu truth")

    assert "bonus reset to zero" in prompt
...
def test_build_script_prompt_aligns_with_script_score_pass_rubric() -> None:
    prompt = build_script_prompt("full-30s", "Jian Ji Shi discover client Yan Shou file be Gai")
    ...
    assert "character Biao Qian" in prompt
...
def test_build_script_prompt_includes_repair_notes_for_retry() -> None:
    prompt = build_script_prompt(
        "full-30s",
        "Jian Ji Shi discover client Yan Shou file be Gai",
        repair_notes=[
            "character Bian Shi Du Bu Zu：protagonist Que Shao fixed Kou Tou Chan",
            "logic Yi Zhi Xing have Lou Dong：Yin Cang Mi Ma Que Shao Qian Zhi Pu Dian",
        ],
    )
...
def test_build_script_prompt_blocks_unseeded_backdoor_solutions() -> None:
    ...
    assert "do not put delete complete" in prompt
...
def test_provider_chain_script_text_preserves_scene_conflict_metadata() -> None:
    ...
    assert "【conflict】issue：who Xiao Lan bonus countdown reset to zero？" in text
    ...
    assert "Zhuan Zhe：permission Ju Jue after，Ri Zhi Fan Xiang Tiao Chu Cao Zuo Zhe Bian Hao。" in text
...
def test_provider_chain_script_text_preserves_all_character_roles() -> None:
    ...
    assert "▲character Biao Qian：Xiaolan｜protagonist｜Lan Se Ka Tong robot，Cheng Se Wei Jin" in text
    assert "▲character Biao Qian：Hong Dun｜calm Shen He Yuan Yao Wei Hu permission Gui Ze" in text
