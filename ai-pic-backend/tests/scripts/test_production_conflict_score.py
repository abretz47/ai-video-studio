...
def test_structured_score_requires_provider_scene_question_and_turn() -> None:
 ...
 scene["dialogue"][0]["line"] = f"Chang{scene_index}Xian Cha Shui"
 for beat_index, beat in enumerate(scene["beats"], start=1):
 beat["dialogue"][0]["line"] = f"Xian Suo{scene_index}{beat_index}Bian Le"
 scene["question"] = "Tui Jin Ju Qing"
 scene["turn"] = ""
...
def test_structured_score_rejects_abstract_provider_stakes_and_opposition() -> None:
 ...
 scene["stakes"] = "Xiaolan Ya Li Yue Lai Yue Da"
 scene["opposition"] = "Hun Luan Ju Mian Zu Zhi Xiaolan continue Cha"
 payload["key_artifacts"]["script"]["raw_content"] = json.dumps(
 script, ensure_ascii=False
)
...
def test_structured_score_accepts_hidden_modifier_as_concrete_opposition() -> None:
 ...
 script = json.loads(payload["key_artifacts"]["script"]["raw_content"])
 for scene in script["scenes"]:
 scene["opposition"] = "Yin Cang De Xiu Gai Zhe Lan Zhu Xiaolan continue Zhui Cha"
 payload["key_artifacts"]["script"]["raw_content"] = json.dumps(
 script, ensure_ascii=False
)
