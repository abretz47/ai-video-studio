...
def _large_timeline_spec(episode: Episode, script: Script, *, clip_count: int) -> dict:
 ...
 dialogue_text = f"robot: Di {ordinal} Ge Shi Jian Zhou Jie Dian."
 ...
 **common,
 ...
 "track_type": "video",
 "text": f"robot check Di {ordinal} Ge Fa Guang Jie Dian.",
 "beat_type": "dialogue",
 "placeholder": True,
 "asset_ref": None,
 }
...
def _valid_shot_for_clip(clip: dict) -> dict:
 ...
 "dialogue_source": f"robot: {clip_id}",
 "visual_prompt": f"3D cartoon robot studies {clip_id}",
 "video_prompt": (
 f"Plot: robot studies {clip_id}. Dialogue: robot: {clip_id}. "
