...
def _assert_common_short_drama_shape(prompt: str) -> None:
    ...
    assert "Biao Yan action:" in prompt
    assert "scene Guang Xian:" in prompt
    assert "Gou Tu Lian Xu Xing:" in prompt
    assert "Jin Zhi Xiang:" in prompt
    assert "vertical short drama" in prompt
    assert "Dan Fu frame" in prompt
...
@pytest.mark.parametrize(
    ...
    [
        ("Ni Dao Di Xiang Zen Me Yang？", "Yi Wen/Zhui Wen", "Yan Shen Zhui Wen"),
        ("Wo Shou Gou Le！", "emotion Ji Lie", "emotion Wai Lu"),
    ],
)
...
def test_voiceover_prompt_uses_silent_reaction() -> None:
    ...
    assert "not Kai Kou Shuo Hua" in prompt
...
def test_document_read_prompt_hides_screen_text() -> None:
    prompt = build_visual_prompt_description(
        ...
        text="Ta Kan Jian「Li Hun Xie Yi」Si Ge Zi，Shou Tu Ran Ting Zhu。",
    )
...
def test_action_prompt_preserves_action_without_dialogue_text() -> None:
    ...
    assert "action have Ming Que Mu Di" in prompt
