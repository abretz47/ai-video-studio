...
def test_clip_storyboard_context_does_not_guess_between_multiple_characters(
    ...
    _add_story_character(db_session, episode.story_id, user.id, "Chen Zhe")
    ...
    result = build_clip_storyboard_context(
        db_session,
        timeline=timeline,
        clip={"clip_id": "clip-1", "track_type": "video", "text": "rainy night doorway"},
        panels=[{"panel_id": "panel-1", "visual_prompt": "rainy night doorway"}],
...
def test_clip_storyboard_context_binds_single_story_character(db_session):
    user, episode, script = bootstrap_episode(db_session)
    virtual_ip = _add_story_character(db_session, episode.story_id, user.id, "Lin Wan")
    timeline = create_timeline(
        db_session,
        episode,
        script,
...
def test_clip_storyboard_context_binds_speaker_alias_signal(db_session):
    user, episode, script = bootstrap_episode(db_session)
    _add_story_character(db_session, episode.story_id, user.id, "Lin Wan")
    chen = _add_story_character(db_session, episode.story_id, user.id, "Chen Zhe")
    ...
    result = build_clip_storyboard_context(
        ...
        clip={
            ...
            "scene_id": "scene_001",
            "beat_id": "beat_001",
            "text": "Che Nei Ce Lian",
        },
        panels=[{"panel_id": "panel-1", "visual_prompt": "Che Nei Ce Lian"}],
        request_reference_images=[],
    )

    assert result.bound_context["characters"][0]["name"] == "Chen Zhe"
...
def _add_story_character(
    ...
    slug = "linwan" if name == "Lin Wan" else "chenzhe"
