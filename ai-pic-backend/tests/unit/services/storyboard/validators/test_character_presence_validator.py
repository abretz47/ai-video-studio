...
class TestDialogueCharacterPresence:
 ...
 """Test narrator/voiceover can be missing from frames."""
 ctx = PipelineContext(script_id=1)
 ctx.scenes = [
 SceneContext(
 scene_number=1,
 dialogues=[
 {"scene_number": 1, "character": "voiceover", "content": "Narrator speaks"},
 {"scene_number": 1, "character": "Alice", "content": "Hello"},
 ],
),
 ]
 ...
 "characters": ["Alice"], # voiceover not in characters - OK
 },
 ]
 ...
 missing_results = [
 r for r in results
 if "missing" in r.message.lower() and "voiceover" in str(r.details)
 ]

 # voiceover should not be reported as missing
 assert len(missing_results) == 0
