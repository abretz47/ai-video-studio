...
class TestScriptScoreService:
 ...
 assert result.verdict == "review"
 assert "Ping Fen Jie Xi failed" in result.risks[0]
 ...
 assert "Zong Fen >= 4.5" in prompt
 assert "Suo You Wei Du >= 4.2" in prompt
 assert "Jing Pin Xian Jiao Zhun Mao Dian" in prompt
 assert "customer Che Dan countdown" in prompt
 assert "Bu Yao Zai Yong"Nan Er Dong Ji Xu Bu Chong"" in prompt
