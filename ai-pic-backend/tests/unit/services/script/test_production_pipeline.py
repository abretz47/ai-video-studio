...
def _scoring(
 ...
 "duration_seconds": 15,
 "hook_type": "reveal",
 "key_line": "Ta Ba evidence Ju Dao camera Qian",
...
def test_build_hook_schedule_uses_marketing_and_episode_beats():
 ...
 {"title": "T", "main_conflict": "Shen Fen Bei Qiang"},
 {
 "summary": "heroine Bei Dang Zhong Xiu Ru",
 "plot_points": [{"description": "heroine Na Chu recording", "timing": "Zhong Duan"}],
 },
 {
 "hook_plan": {
 "opening_hook": "Hun Li Xian Chang betrayal",
 "payoff_plan": "Nv Zhu Bo Fang evidence counterattack",
 },
 ...
 assert schedule["payoff"] == "Nv Zhu Bo Fang evidence counterattack"
...
async def test_production_generation_raises_after_six_low_quality_attempts():
 calls: list[int] = []
 scores = [
 _scoring("rewrite", 2.1, ["Zhong Xie Qian3Miao Gou Zi"]),
 _scoring("rewrite", 3.2, ["Ya Suo Zhong Duan Jie Shi"]),
 _scoring("review", 4.3, ["Bu Qiang Ren Wu Ke Bian Shi Du"]),
 _scoring("pass", 4.4, ["Reng not yet Da Dao Jing Pin Xian"]),
 ...
 assert details["rewrite_guidance"][0] == "Reng not yet Da Dao Jing Pin Xian"
...
def test_annotate_storyboard_frames_with_hooks_uses_traffic_sheet():
 ...
 {"description": "heroine Ju Qi evidence complete counterattack", "beat_type": "dialogue"},
 {"description": "Men Wai suddenly Chu Xian Hei Yi Ren", "beat_type": "action"},
 ]

 changed = annotate_storyboard_frames_with_hooks(
 frames,
 hook_schedule={"opening_hook": "Chong Tu Kai Chang"},
 scoring=_scoring("pass", 4.5),
)

 assert changed >= 2
 assert frames[0]["hook_tag"] == "opening_hook"
 assert frames[0]["ad_snippet"]["hook"] == "Ta Ba evidence Ju Dao camera Qian"
