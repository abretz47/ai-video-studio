...
def test_render_production_requirements_expands_commercial_rewrite_contract():
    requirements = render_production_requirements(
        base_additional_requirements=None,
        hook_schedule={"opening_hook": "client Pai Zhuo Zhi Yi data"},
        rewrite_guidance=["character Bian Shi Du Bu Zu", "logic Yi Zhi Xing have Lou Dong"],
        attempt_no=2,
    )
    ...
    assert "Bu Neng be protagonist Yi Wen Jiu Cheng Ren" in requirements
    assert "Zhu Li Bi Xu have Ju Ming action Biao Qian" in requirements
    assert "Si Xia Dui Zhi He Li Xing" in requirements
    assert "client Zhang total Gei Chu60seconds Che Dan countdown" in requirements
    assert "Shu Zi Bu Hui Sa Huang，Kan time Chuo" in requirements
    assert "Gai Wan Gei Ni20Wan，not Zuo Jiu Cai Ni" in requirements
