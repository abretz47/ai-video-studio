...
class TestCharacterProfile:
 ...
 assert profile.aliases == ["Xiao Bao"]
...
class TestCharacterConsistencyValidator:
 ...
 assert validator.resolve_name("John Doe") == "John Doe"
 assert validator.resolve_name("Jane Roe") == "Jane Roe"

 def test_resolve_name_alias(self, validator):
 """Test resolving alias to canonical name."""
 assert validator.resolve_name("Old Zhang") == "John Doe"
 assert validator.resolve_name("Zhang San") == "John Doe"
 assert validator.resolve_name("Alex Li") == "Jane Roe"
 ...
 assert validator.resolve_name("Passerby A") is None

 def test_get_profile(self, validator):
 """Test getting profile by name or alias."""
 profile = validator.get_profile("John Doe")
 assert profile is not None
 assert profile.name == "John Doe"
 assert profile.gender == "male"

 # Via alias
 profile2 = validator.get_profile("Old Zhang")
 assert profile2 is not None
 assert profile2.name == "John Doe"

 def test_validate_names_in_text_all_known(self, validator):
 """Test validating text with all known characters."""
 text = """
John Doe: Jin Tian Tian Qi Bu Cuo.
Jane Roe: Shi A, Wo Men Qu San Bu Ba.
Wang Mei: Hao Zhu Yi!
"""
 ...
 text = """
Old Zhang: Alex Li, Ni Lai Yi Xia.
Alex Li: Hao De, Old Zhang.
"""
 results = validator.validate_names_in_text(text)
 ...
 text = """
John Doe: Ni Hao.
Stranger: Ni Hao, Wo Shi Stranger.
"""
 results = validator.validate_names_in_text(text)
...
"""
 ...
 text = """
voiceover: Zhe Shi Yi Ge Yang Guang Ming Mei De Zao Chen.
John Doe: Jin Tian Xin Qing Bu Cuo.
...
"""
 ...
 assert not any("voiceover" in str(r.details) for r in unknown_warnings)
 ...
 results = validator.validate_character_attributes(
 "John Doe", # introverted in profile
...
class TestNameSimilarity:
 ...
 assert validator._names_similar("Zhang", "John Doe") is True
...
class TestAgeCompatibility:
 ...
 assert validator._age_compatible("Qing Nian", "Nian Qing") is True
 ...
 assert validator._age_compatible("young", "Lao Nian") is False
...
class TestViolationScenarios:
 ...
 CharacterProfile(
 name="Dr. Lin",
 aliases=["Dr. Lin", "Lin Da Fu"],
 gender="female",
 age="middle-aged",
 personality=["calm", "professional", "calm"],
 voice_style="authoritative",
),
 CharacterProfile(
 name="Xiao Ming",
 aliases=["Ming", "Ming Ming"],
 gender="male",
 age="child",
 ...
 CharacterProfile(
 name="Wang Lao Ban",
 aliases=["Boss Wang", "Wang Zong"],
 gender="male",
 age="elderly",
 personality=["shrewd", "aggressive", "Bao Zao"],
),
 ]
)
 return v
 ...
 "personality": ["aggressive", "Bao Zao"], # Should be calm
 },
)
 ...
 results = validator.validate_character_attributes(
 "Wang Lao Ban", # Bao Zao (aggressive)
 {"personality": ["calm"]}, # calm - opposite
)
 ...
 text = """
[Dr. Lin Zou Jin office]
Dr. Lin: Qing Zuo.
[Xiao Ming Zhan Qi Lai Kan Zhe Chuang Wai]
Xiao Ming: Hao De, doctor.
"""
 results = validator.validate_names_in_text(text)
...
Dr. Lin: you Jin Tian Gan Jue Zen Me Yang?
"""
 results = validator.validate_names_in_text(text)
 ...
 text = """
Dr. Lin: Ni Hao.
Shen Mi Ren: Wo You Zhong Yao De Shi Gao Su you.
Xiao Ming: Na Shi Shui?
"""
 results = validator.validate_names_in_text(text)

 warnings = [r for r in results if r.severity == ValidationSeverity.WARNING]
 assert len(warnings) >= 1
 assert any("Shen Mi Ren" in str(r.details) for r in warnings)

 def test_multiple_unknown_characters(self, validator):
 """Test detecting multiple unknown characters."""
 text = """
Passerby A: you Kan Jian Le Ma?
Lu Ren Yi: Shi De, Tai Jing Ren Le.
Lu Ren Bing: Wo Men fast Zou Ba.
"""
 results = validator.validate_names_in_text(text)
...
"""
 ...
 results = validator.validate_character_attributes(
 "Alice",
 {"gender": "female"}, # Chinese female matches English female
)
 assert all(r.passed for r in results)
 ...
 results = validator.validate_character_attributes(
 "Alice",
 {"age": "Qing Nian"}, # youth matches young
)
 assert all(r.passed for r in results)
 ...
 text = """
voiceover: Zhe Shi Yi Ge Qing Lang De Zao Chen.
Narrator: The story begins here.
Hua Wai Yin: Yuan Chu Chuan Lai Le Sheng Yin.
...
"""
 ...
 warnings = [r for r in results if r.severity == ValidationSeverity.WARNING]
 assert not any("voiceover" in str(r.details) for r in warnings)
 assert not any("Narrator" in str(r.details) for r in warnings)
 assert not any("Hua Wai Yin" in str(r.details) for r in warnings)
