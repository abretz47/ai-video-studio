...
class TestSceneContextBuilding:
 ...
 assert infer_conflict_level("Ji Lie De Dui Zhi") == "high"
 assert infer_conflict_level("tense De Chang Mian") == "high"

 def test_infer_conflict_level_low(self):
 """Should detect low conflict keywords."""
 assert infer_conflict_level("calm De Zao Chen") == "low"
 assert infer_conflict_level("Wen Xin De families") == "low"

 def test_infer_conflict_level_default(self):
 """Should return medium for unknown."""
 assert infer_conflict_level(None) == "medium"
 assert infer_conflict_level("Pu Tong Dui Hua") == "medium"
...
class TestDialogueContextBuilding:
 ...
 dialogues = [
 {"character": "Xiao Ming", "content": "Ni Hao", "emotion": "happy"},
 {"character": "Xiao Hong", "content": "Ni Ye Hao", "emotion": "calm"},
 ]
 contexts = build_dialogue_contexts(dialogues)

 assert len(contexts) == 2
 assert contexts[0].speaker == "Xiao Ming"
 assert contexts[0].emotion == "happy"
 assert contexts[0].prev_emotion is None
 assert contexts[0].is_first is True

 assert contexts[1].speaker == "Xiao Hong"
 ...
 scene_number=1,
 dialogues=dialogues,
 conflict_notes="Ji Lie Zheng Chao",
)
