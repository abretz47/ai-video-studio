...
class TestStoryContext:
    ...
    def test_pack_with_budget(self):
        ...
        ctx = StoryContext(
            ...
            setting="Zhe Shi Yi Ge Fei Chang Xiang Xi Bei Jing She Ding" * 100,
        )
...
class TestEpisodeContext:
    ...
    def test_valid_context(self):
        """Should pass with all required fields."""
        ctx = EpisodeContext(
            story_outline="story outline content",
            episode_number=1,
        )
        errors = ctx.validate()
        assert errors == []
...
class TestTokenBudgetScenarios:
    ...
    def test_episode_context_tight_budget(self):
        ...
        ctx = EpisodeContext(
            story_outline="Zhe Shi story outline",  # Short critical field
            episode_number=5,
            total_episodes=10,
            target_duration=60,
