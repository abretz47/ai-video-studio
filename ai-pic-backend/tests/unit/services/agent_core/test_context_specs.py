...
class TestStoryContext:
 ...
 setting="Zhe Shi Yi Ge Fei Chang Xiang Xi De Bei Jing She Ding" * 100,
)
...
class TestEpisodeContext:
 ...
 """Should pass with all required fields."""
 ctx = EpisodeContext(
 story_outline="story Da Gang content",
 episode_number=1,
)
 errors = ctx.validate()
 assert errors == []
 ...
 ctx = EpisodeContext(
 story_outline="Da Gang",
 episode_number=1,
 continuity_ledger="Lian Xu Xing" * 500, # Long
)
...
class TestStoryboardContext:
 ...
 ctx = StoryboardContext(
 script_beat="characterAZou Jin Fang Jian, Kan Xiang Chuang Wai",
)
 errors = ctx.validate()
 assert errors == []
...
class TestTokenBudgetScenarios:
 ...
 title="Ce Shi Gu Shi",
 genre="suspense",
 characters=[{"name": "Protagonist", "desc": "30Sui Nan Xing"}],
 setting="Xian Dai Du Shi",
 theme="Cheng Zhang",
 tone="tense",
 constraints="none Xue Xing violence",
 hook_requirements="Mei Ji Jie Wei Liu Xuan Nian",
 target_duration=300,
 episode_count=10,
)
 ...
 ctx = EpisodeContext(
 story_outline="Zhe Shi story Da Gang", # Short critical field
 episode_number=5,
 total_episodes=10,
 target_duration=60,
 characters=[{"name": "A"}, {"name": "B"}],
 continuity_ledger={"facts": ["fact1"] * 100}, # Long non-critical
 previous_summary="Shang Ji Hui Gu" * 50, # Long low-priority
 arc_requirements="characterAXu Yao Cheng Zhang",
 hook_plan="Ben Ji Xuan Nian",
)
 packed = ctx.pack(max_tokens=100)
