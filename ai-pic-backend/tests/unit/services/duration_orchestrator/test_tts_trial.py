...
test Duration Orchestrator   TTS when Zhang Gu Suan logic。
"""
...
class TestEstimateDurationFromDialogues:
    """test dialogue when Zhang Gu Suan"""

    def test_estimate_normal_dialogues(self):
        """normal dialogue duration Gu Suan"""
        dialogues = [
    ...
    def test_estimate_long_dialogue(self):
        """Zhang dialogue duration Gu Suan"""
...
class TestTtsTrialNode:
    ...
    async def test_updates_reasoning(self, base_state):
        ...
        assert "word count Gu Suan" in result["reasoning"][0]

    @pytest.mark.asyncio
    async def test_no_dialogues(self, base_state):
        """none dialogue when return Kong"""
    ...
    async def test_actual_tts_mode_fallback(self, base_state):
        """actual TTS Mo Shi Jiang Ji for Gu Suan"""
        base_state["use_actual_tts"] = True
        # Mei You tts_service，Ying Gai Jiang Ji for Gu Suan Mo Shi
    ...
    async def test_actual_tts_mode_with_service(self, base_state):
        """actual TTS Mo Shi have service"""
