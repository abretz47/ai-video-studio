...
test Duration Orchestrator scene Ti Jiao and Yu Suan Zai Ping Heng logic。
"""
...
class TestCommitSceneNode:
    ...
    def test_commit_scene_status(self, base_state):
        """Ti Jiao after status Bian Wei COMMITTED"""
    ...
    def test_commit_saves_scene_data(self, base_state):
        """Ti Jiao after Bao Cun scene data"""
        result = commit_scene_node(base_state)
...
class TestShouldContinueOrAssemble:
    ...
    def test_continue_when_pending_scenes(self):
        """Hai You Dai Chu Li scene when continue"""
        state = {
            "scene_budgets": [
                SceneBudget(
                    scene_number=1,
    ...
    def test_assemble_when_all_committed(self):
        """Suo You scene already Ti Jiao when Jin Ru Zu Zhuang"""
        state = {
            "scene_budgets": [
                SceneBudget(
                    scene_number=1,
    ...
    def test_assemble_when_index_exceeds(self):
        """Suo Yin Chao Chu when Jin Ru Zu Zhuang"""
        state = {
            "scene_budgets": [
                SceneBudget(
                    scene_number=1,
