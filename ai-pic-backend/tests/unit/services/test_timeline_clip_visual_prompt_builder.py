...
def clip_with_motion_plan() -> dict:
    return {
        ...
        "track_type": "video",
        "text": "Chen Zhe Zuo Zai Che INT，Ce Lian be phone Ping Zhao Liang。",
        "source_refs": {
            "timeline_shot_plan": {
                ...
                "direction_anchor": "rainy night Che Nei Chi Yi and Jing Jue",
                ...
                "motion_timeline": [
                    {"at_ms": 0, "action": "Ta Di Tou Ding phone screen"},
                    {"at_ms": 900, "action": "Ta Shou Zhi Ting in Bo Hao Jian on"},
                    {"at_ms": 1800, "action": "Ta Tai Yan Kan Xiang Yu in Che Chuang"},
                ],
                "emotional_landing": "Ke Zhi Huai Yi Luo Dian",
            }
        },
    }
...
def test_keyframe_prompts_use_first_and_last_motion_points() -> None:
    ...
    assert "Ta Di Tou Ding phone screen" in frames[0]["prompt"]
    assert "Ta Tai Yan Kan Xiang Yu in Che Chuang" in frames[1]["prompt"]
...
def test_video_motion_prompt_prefers_operator_override() -> None:
    prompt, metadata = build_timeline_clip_video_motion_prompt(
        ...
        override="only Bao Liu Qing Wei Hu Xi and Chuang Wai Yu Di，camera fixed",
    )

    assert "only Bao Liu Qing Wei Hu Xi and Chuang Wai Yu Di" in prompt
...
def test_video_motion_prompt_without_override_keeps_motion_and_constraints() -> None:
    ...
    assert "0ms: Ta Di Tou Ding phone screen" in prompt
    assert "1800ms: Ta Tai Yan Kan Xiang Yu in Che Chuang" in prompt
