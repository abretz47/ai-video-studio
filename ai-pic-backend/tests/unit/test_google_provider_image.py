...
async def test_generate_image_success(monkeypatch):
    ...
    # validate responseModalities Zi Duan already correct set
...
async def test_generate_image_strips_provider_prefix(monkeypatch):
    """test model name in provider Qian Zhui be correct clean up"""
    provider = GoogleProvider(ProviderConfig(name="google", api_key="test-key"))
    dummy_client = _DummyClient(
        {
            "candidates": [
    ...
    # use Dai You provider Qian Zhui model name
    resp = await provider.generate_image(
        prompt="test image", model="google:gemini-3-pro-image-preview"
    )

    assert resp.success is True
    # validate URL in model name not Bao Han provider Qian Zhui
