...
def test_canvas_skill_plan_selects_existing_ip_and_environment_pool(db_session):
 ...
 user_id=user.id,
 name="Lin Mei",
 description="Cheng Xu Yuan Shen Bian De light comedy heroine",
 ...
 description="Shi He Cheng Xu Yuan short drama De Ming Liang Ban Gong environment",
...
def test_canvas_skill_plan_does_not_fallback_to_unmatched_assets(db_session):
 ...
 assert plan.nodes[1].title == "Dai Que Ren IP He environment Zi Chan"
...
def test_canvas_skill_plan_can_select_environment_without_ip(db_session):
 ...
 assert plan.nodes[1].title == "IP Dai Que Ren; Yi Xuan Ze environment: office"
 assert plan.nodes[1].outputs["environment_ids"] == [environment.id]
...
def test_canvas_skill_plan_carries_script_context_for_downstream_execution(
 ...
 user,
 ProductionCanvasPlanRequest(
 prompt="based on Xian You script continue Sheng Cheng Fen Jing He time Xian",
 episode_id=episode.id,
 script_id=script.id,
),
)
