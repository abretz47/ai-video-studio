...
def test_storyboard_image_prompt_template_forbids_collage():
    base_prompt = "Old Guai, Zhi Xing Zui Zhong Ya Li test，Fu Zai Ti Sheng Zhi Feng Zhi。 Jing Bie: medium shot Yun Jing: fixed Gou Tu: San Fen Fa camera Qie Huan: opening"
...
def test_storyboard_grid_sheet_template_allows_sheet_but_limits_text():
    prompt = prompt_manager.render_prompt(
        PromptTemplate.STORYBOARD_GRID_SHEET.value,
        {
            ...
            "panel_briefs": [
                "Panel 1 / clip clip-1: Lin Wan Zhan in rainy night doorway，Ni Hong Fan Guang，medium shot",
                "Panel 2 / clip clip-2: Chen Zhe Zuo Zai Che INT，Ce Lian be phone Ping Zhao Liang",
            ],
        },
    )
    ...
    assert "Lin Wan Zhan in rainy night doorway" in prompt
...
def test_storyboard_keyframe_template_forbids_collage():
    prompt = prompt_manager.render_prompt(
        ...
        {"base_prompt": "Ye Se in office，Jing Bie: medium shot", "role": "start"},
    )
...
def test_storyboard_scene_template_forbids_on_screen_text():
    ...
    assert "Bu Yao Bao Han Ren He“frame in Chu Xian Wen Zi content”" in prompt
...
def test_storyboard_audio_visual_prompt_templates_exist():
    ...
    prompt = prompt_manager.render_prompt(
        ...
        {"action": "Ta Meng Di file Pai in Zhuo Shang，Nu Shi Dui Fang"},
    )
    assert "Pai in Zhuo Shang" in prompt
    assert "no subtitles" in prompt
    ...
    prompt = prompt_manager.render_prompt(
        "storyboard_audio_visual_context",
        {
            ...
            "environment": "Gong Yu living room，night",
        },
    )
    assert "character Ka" in prompt
    assert "environment Mao Dian" in prompt
