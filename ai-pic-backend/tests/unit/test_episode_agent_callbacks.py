...
async def test_episode_agent_callbacks_emit_and_fallback(monkeypatch):
    ...
    assert any(msg.startswith("episode outline：") for msg in progress)
    assert any(msg == "generate Di1Ji：call model" for msg in progress)
    assert any(msg == "generate Di2Ji：call model" for msg in progress)
    ...
    episode_prompts = [
        ...
        if isinstance(p, str) and "Dang Qian Ji Step Outline" in p
    ]
    assert episode_prompts
    assert "Zhong Dian character：Wen Wen" in episode_prompts[0]
