...
async def test_episode_agent_callbacks_emit_and_fallback(monkeypatch):
 ...
 focus_characters=[{"id": 1, "name": "Wen Wen", "description": "desc"}],
 ...
 assert any(msg.startswith("Ju Ji Da Gang: ") for msg in progress)
 assert any(msg == "Sheng Cheng Di1Ji: Diao Yong Mo Xing" for msg in progress)
 assert any(msg == "Sheng Cheng Di2Ji: Diao Yong Mo Xing" for msg in progress)
 ...
 if isinstance(p, str) and "Dang Qian Ji De Step Outline" in p
 ]
 assert episode_prompts
 assert "Zhong Dian Jue Se: Wen Wen" in episode_prompts[0]
...
async def test_episode_agent_validates_character_profiles(monkeypatch):
 ...
 step_outlines={"episodes": [{"episode_number": 1, "logline": "Lin Xue Xing Dong"}]},
 raw_text="{}",
 ...
 {
 "episode_number": 1,
 "summary": "Lin Xue discover Xian Suo.",
 "plot_points": [{"description": "Lin Xue Xing Dong", "timing": "opening"}],
 "characters": [{"name": "Lin Xue"}],
 }
 ],
 provider="mock",
 model="mock-model",
 ...
 agent = EpisodeLangGraphAgent(Svc())
 result = await agent.generate(
 story={"character_profiles": [{"name": "Lin Xue"}]},
 episode_count=1,
 episode_duration=None,
