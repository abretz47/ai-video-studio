...
async def test_ensure_virtual_ip_voice_config_fallback_when_ai_missing(
 ...
 vip = VirtualIP(name="Ce Shi Jue Se", description="Yong Gan De young adults")
 db_session.add(vip)
 db_session.commit()
...
async def test_ensure_derived_character_voice_binding_fallback_scope_story(
 ...
 episode_id=ep2.id,
 title="S2",
 dialogues=[{"scene_number": 1, "character": "clerk", "content": "Zhe Bian Qing"}],
)
)
 db_session.commit()
 ...
 scene=None,
 script_dialogues=[
 {"scene_number": 1, "character": "clerk", "content": "Zhe Bian Qing"}
 ],
