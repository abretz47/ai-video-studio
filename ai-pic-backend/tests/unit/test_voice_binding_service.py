...
async def test_ensure_virtual_ip_voice_config_fallback_when_ai_missing(
    ...
    vip = VirtualIP(name="test character", description="Yong Gan young adults")
    db_session.add(vip)
    db_session.commit()
...
async def test_ensure_derived_character_voice_binding_fallback_scope_episode(
    ...
    script = Script(
        ...
        title="Script",
        dialogues=[
            {"scene_number": 1, "character": "passerby", "content": "Ni Hao"},
            {"scene_number": 1, "character": "passerby", "content": "Zai Jian"},
        ],
    )
...
async def test_ensure_derived_character_voice_binding_fallback_scope_story(
    ...
    db_session.add(
        Script(
            episode_id=ep1.id,
            title="S1",
            dialogues=[{"scene_number": 1, "character": "clerk", "content": "Huan Ying Guang Lin"}],
        )
    )
    ...
    db_session.add(
        Script(
            episode_id=ep2.id,
            title="S2",
            dialogues=[{"scene_number": 1, "character": "clerk", "content": "Zhe Bian please"}],
        )
    )
    db_session.commit()
    ...
    scope, cfg = await ensure_derived_character_voice_binding(
        ...
        scene=None,
        script_dialogues=[
            {"scene_number": 1, "character": "clerk", "content": "Zhe Bian please"}
        ],
        character_name="clerk",
    )
    assert scope == "story"
    assert cfg.get("voice_id")
...
async def test_ensure_derived_character_voice_binding_fallback_scope_scene(
    ...
    scope, cfg = await ensure_derived_character_voice_binding(
        ...
        scene=scene,
        script_dialogues=[
            {"scene_number": 1, "character": "Passerby A", "content": "（Chen Mo）"}
        ],
        character_name="Passerby A",
    )
    assert scope == "scene"
    assert cfg.get("voice_id")
