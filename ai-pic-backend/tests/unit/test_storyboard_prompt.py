...
def test_build_storyboard_context_includes_scene_details():
    script_payload = {
        ...
        "scene_indices": [1, 2],
        "dialogues": [
            {"scene_number": 1, "content": "Alex Li：this only Shou Lian Duo Shao Qian？"},
        ...
        "stage_directions": [
            ...
            {"scene_number": 2, "content": "Bei Guang Jian Ying，slow Tui"},
        ],
    ...
    assert "story Bei Jing" in context
    assert "episode information" in context
    assert "scene 1" in context
    assert "location:Night Market Main Street" in context
    assert "dialogue:Alex Li：this only Shou Lian Duo Shao Qian？" in context
