...
Zhi Jie testOpenAI APIcall，not Yi Lai completeFastAPIYing Yong
"""
...
if os.getenv("RUN_OPENAI_DIRECT_TEST") != "1":
    pytest.skip(
        "OpenAI Zhi Lian API Mao Yan test（Mo Ren Tiao Guo）；set RUN_OPENAI_DIRECT_TEST=1 Cai run",
...
async def test_openai_dalle_direct(skip_if_no_openai):
    """Zhi Jie testOpenAI DALL-E APIcall"""
    ...
    if not settings.OPENAI_API_KEY:
        pytest.skip("OPENAI_API_KEY Wei configuration")
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            print(f"⏱️ APIcall Hao Shi: {duration:.2f}seconds")
            print(f"📡 HTTPstatus Ma: {response.status_code}")
            ...
            if "data" in result and len(result["data"]) > 0:
                ...
                if "b64_json" in data:
                    ...
                    print(f"   base64data Chang Du: {len(base64_data)}")
                    ...
                    try:
                        ...
                        print(f"   Jie Ma after image Da Xiao: {len(decoded)} bytes")
                        print(
                        ...
                        print(f"   ❌ base64Jie Ma failed: {decode_error}")
                        raise
                else:
                    print("   ❌ response in Mei Youb64_jsonZi Duan")
                    raise AssertionError("missing b64_json")
            else:
                print("   ❌ response in Mei YoudataZi Duan")
                raise AssertionError("missing data")

    except httpx.TimeoutException as e:
        print(f"❌ Qing Qiu timeout: {e}")
        raise
