...
class TestNotFoundError:
    ...
    def test_basic_not_found(self):
        ...
        assert error.message == "user not exists: 123"
    ...
    def test_not_found_without_id(self):
        ...
        assert error.message == "virtualIPnot exists"
    ...
    def test_virtual_ip_factory(self):
        ...
        assert error.message == "virtualIPnot exists: vip_123"

    def test_script_factory(self):
        error = NotFoundError.script(456)
        assert error.message == "Jiao Ben not exists: 456"
...
class TestValidationError:
    ...
    def test_basic_validation_error(self):
        error = ValidationError("virtualIPname already exists")
        assert error.message == "virtualIPname already exists"
        assert error.status_code == 400
        assert error.error_code == "VALIDATION_ERROR"

    def test_validation_error_with_field(self):
        error = ValidationError("format not correct", field="email")
        assert error.context["field"] == "email"

    def test_missing_field_error(self):
        error = MissingFieldError("user_id")
        assert error.message == "Que Shao Bi Tian Zi Duan: user_id"
        assert error.status_code == 400
        assert error.context["field"] == "user_id"

    def test_missing_field_custom_message(self):
        error = MissingFieldError("prompt", "Bi Xu Ti Gong prompt Ci")
        assert error.message == "Bi Xu Ti Gong prompt Ci"

    def test_invalid_format_error(self):
        error = InvalidFormatError("scene_numbers")
        assert error.message == "Zi Duan format not correct: scene_numbers"
        assert error.context["field"] == "scene_numbers"

    def test_invalid_format_custom_message(self):
        error = InvalidFormatError("date", "Ri Qi format Bi Xu forYYYY-MM-DD")
        assert error.message == "Ri Qi format Bi Xu forYYYY-MM-DD"

    def test_duplicate_error(self):
        error = DuplicateError("virtualIPname", "test name")
        assert error.message == "virtualIPname already exists: test name"
        assert error.context["value"] == "test name"
...
class TestAuthErrors:
    ...
    def test_conflict_error(self):
        error = ConflictError("Zi Yuan Zheng Zai be use")
        assert error.message == "Zi Yuan Zheng Zai be use"
...
class TestServiceErrors:
    ...
    def test_service_error(self):
        error = ServiceError("Nei Bu error")
        assert error.status_code == 500
    ...
    def test_configuration_error(self):
        error = ConfigurationError("OSSservice Wei configuration")
        assert error.message == "OSSservice Wei configuration"
        assert error.status_code == 500

    def test_external_service_error(self):
        error = ExternalServiceError("OpenAI", "connection timeout")
        assert error.message == "Wai Bu service OpenAI unavailable: connection timeout"
    ...
    def test_external_service_no_reason(self):
        ...
        assert error.message == "Wai Bu service Keling unavailable"
...
class TestExceptionToDict:
    ...
    def test_simple_exception_to_dict(self):
        ...
        assert result["message"] == "user not exists: 123"
