...
Zhuan Zhu Yu testUserManagementServiceDe Ye Wu logic
"""
...
class TestUserApproval(TestUserManagementService):
 ...
 """test approval Bu exists user failed"""
 admin = self.create_mock_admin()
 ...
 assert "user Bu exists" in str(exc_info.value.detail)
 ...
 assert "Wu Xiao De operation type" in str(exc_info.value.detail)
...
class TestRoleManagement(TestUserManagementService):
 ...
 """test update user character success"""
 ...
 """test Bu Neng Xiu Gai Zi Ji De permission"""
 user = self.create_mock_user(id=1)
 self.db.query.return_value.filter.return_value.first.return_value = user
 ...
 assert exc_info.value.status_code == 400
 assert "Bu Neng Xiu Gai Zi Ji De permission" in str(exc_info.value.detail)
...
class TestUserSuspension(TestUserManagementService):
 ...
 assert exc_info.value.status_code == 400
 assert "Bu Neng suspend Zi Ji De Zhang Hu" in str(exc_info.value.detail)

 def test_reactivate_user_success(self):
 """test reactivate activate user success"""
 user = self.create_mock_user(
 id=1,
 is_active=False,
...
class TestUserDeletion(TestUserManagementService):
 ...
 assert exc_info.value.status_code == 400
 assert "Bu Neng delete Zi Ji De Zhang Hu" in str(exc_info.value.detail)
...
class TestUserStats(TestUserManagementService):
 ...
 """test get user statistics success"""
...
class TestActivationToken(TestUserManagementService):
 ...
 """test generate activate Ling Pai success"""
 ...
 """test validate activate Ling Pai success"""
 user = self.create_mock_user(
...
class TestLoginAttempts(TestUserManagementService):
 ...
 """test increase failed login Ci Shu"""
 ...
 """test5Ci failed Hou Suo Ding Zhang Hu"""
...
class TestAuditLog(TestUserManagementService):
 ...
 """test create Shen Ji Ri Zhi success"""
...
class TestUserSearch(TestUserManagementService):
 ...
 """test Dai Shai Xuan Tiao Jian De user list"""
 mock_users = [Mock(), Mock()]
 query_mock = self.db.query.return_value
 query_mock.filter.return_value = query_mock # Lian Shi Diao Yong
 query_mock.count.return_value = 2
 query_mock.order_by.return_value.offset.return_value.limit.return_value.all.return_value = (
 mock_users
)
 ...
 assert len(users) == 2
 assert total == 2
 # validate Guo Lv Qi Bei Ying Yong(Ju Ti Defiltercall Ci Shu Qu Jue Yu Shi Xian)
