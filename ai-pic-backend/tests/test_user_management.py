...
class TestUserRegistration:
 ...
 # Shou user create administrator, Xian Qing Li
 ...
 # Shou Ge user Zi Dong Jin Sheng Wei administrator Bing activate
 ...
 """test Di Er Ge user keep pending approval status"""
 db_session.query(User).delete()
 db_session.commit()

 # Shou Ge Yong Hu -> administrator
 client.post(
 ...
 # Chang Shi Yong Xiang Tong Yong Hu Ming create Di Er Ge user
 user2_data = {
 "username": "duplicate",
 "email": "user2@example.com",
 "password": "pass123",
 ...
 assert "Yong Hu Ming Yi exists" in response.json()["detail"]
 ...
 response = client.post("/api/v1/auth/register", json=user2_data)
 assert response.status_code == 400
 assert "You Xiang Yi exists" in response.json()["detail"]
...
class TestUserLogin:
 ...
 """create Yi approval De test user"""
 user = User(
 username=username,
 ...
 """create pending approval De test user"""
 user = User(
 username=username,
 email=f"{username}@example.com",
 hashed_password=get_password_hash("testpass123"),
 ...
 """test Yi approval user login success"""
 ...
 """test pending approval user login failed"""
 ...
 assert "Yong Hu Ming Huo Mi Ma error" in response.json()["detail"]

 def test_login_increments_failed_attempts(self, db_session: Session, client):
 """test failed login increase failed Ci Shu"""
 user = self.create_approved_user(db_session)
 ...
 """test5Ci failed Hou Zhang Hu Suo Ding"""
 user = self.create_approved_user(db_session)
...
class TestUserManagementService:
 ...
 # create Bu Tong status De user
 self.create_admin_user(db_session)

 # create Ji Ge Bu Tong status De user Yong Yu statistics
 for i in range(3):
 user = User(
...
class TestMiddleware:
 ...
 # Chang Shi access Xu Yao Huo Yue user permission De Jie Kou
 response = client.get(
 ...
 """test administrator Zhong Jian Jian Zu Zhi Pu Tong Yong Hu"""
 user = User(
 username="regular",
 email="regular@example.com",
 hashed_password=get_password_hash("pass"),
