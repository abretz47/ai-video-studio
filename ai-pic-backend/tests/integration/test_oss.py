...
def test_oss_connection():
    """testOSSJi Chu connection"""
    print("🔍 testOSSconnection...")

    # Jia Zai configuration
    ...
    if not all(
        ...
        print("❌ OSSconfiguration not complete")
        return False

    try:
        # createOSSRen Zheng andbucketobject
        auth = oss2.Auth(
        ...
        # test1: Lie Chubucketin file（Zhi Qu before5Ge）
        print("\n🔍 test Lie Chu file...")
        result = bucket.list_objects(max_keys=5)
        print(f"✅ Cheng Gong connection，Zhao Dao {len(result.object_list)} Ge object")

        # test2: upload Yi Ge Jian Dan Wen Ben Wen Jian
        print("\n🔍 test upload file...")
        test_content = f"OSStest file - {datetime.now().isoformat()}"
        test_key = f"test/oss_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"

        bucket.put_object(test_key, test_content)
        print(f"✅ Cheng Gong upload test file: {test_key}")

        # test3: Xia Zai file validate
        print("\n🔍 test Xia Zai file...")
        downloaded = bucket.get_object(test_key)
        content = downloaded.read().decode("utf-8")
        print(f"✅ Cheng Gong Xia Zai，content Pi Pei: {content == test_content}")
        ...
        print(f"❌ OSSconnection failed: {str(e)}")
        return False


def test_oss_with_metadata():
    """test Daimetadataupload"""
    print("\n🔍 test Daimetadataupload...")
    ...
    try:
        ...
        # test Bu Daimetadataupload
        test_content = "test Bu Daimetadata"
        test_key = f"test/no_metadata_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        bucket.put_object(test_key, test_content)
        print(f"✅ nonemetadataupload Cheng Gong: {test_key}")
        bucket.delete_object(test_key)

        # test DaiASCII metadataupload
        ...
        print(f"✅ ASCII metadataupload Cheng Gong: {test_key}")
        bucket.delete_object(test_key)
...
if __name__ == "__main__":
    ...
    if test_oss_connection():
        ...
        print("\n💡 Ke Neng Jie Jue Fang An:")
        print("1. checkOSS Access KeyandSecret KeyShi Fou correct")
