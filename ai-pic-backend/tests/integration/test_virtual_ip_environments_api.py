...
def _virtual_ip(db, user: User, name: str = "character") -> VirtualIP:
...
def _environment(db, user: User, name: str = "environment") -> Environment:
 env = Environment(
 user_id=user.id,
 name=name,
 category="indoor",
 description="Ke Fu Yong scene Zi Chan",
)
 db.add(env)
 db.commit()
 db.refresh(env)
...
def test_link_update_unlink_virtual_ip_environment(client, db_session):
 ...
 json={"usage_note": "Zhu Chang Jing", "is_default": True},
)
 assert updated.status_code == 200
 assert updated.json()["usage_note"] == "Zhu Chang Jing"
...
def test_environment_response_includes_linked_virtual_ips(client, db_session):
 user = db_session.query(User).filter(User.username == "test_admin").first()
 vip = _virtual_ip(db_session, user, "Lin Xue")
 env = _environment(db_session, user, "Gong Yu")
 ...
 assert payload["linked_virtual_ips"][0]["name"] == "Lin Xue"
