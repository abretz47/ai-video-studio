...
def test_virtual_ip(db: Session, test_user: User):
    ...
    vip = VirtualIP(
        ...
        biography="test Xing Ge",
        background_story="test Bei Jing",
        style_prompt="test Wai Guan",
        voice_config={
            "provider": "minimax",
...
class TestEpisodeCharacterCRUD:
    ...
    def test_create_episode_character(
        ...
        data = response.json()
        assert data["character_name"] == "courier Yuan"
...
class TestCharacterExtraction:
    ...
    def test_extract_from_dialogues(self):
        ...
        script_content = {
            "dialogues": [
                {"character": "courier Yuan", "content": "Nin courier to", "scene_number": 3},
                {"character": "courier Yuan", "content": "please Qian Shou", "scene_number": 3},
        ...
        results = extract_temporary_characters(
            ...
            unknown_names=["courier Yuan", "doctor"],
        )

        assert len(results) == 2
        assert results[0].character_name == "courier Yuan"
    ...
    def test_extract_appearance_hints(self):
        ...
        stage_direction = "wearing courier Zhi Fu，Dai Zhe Kou Zhao，Bei Zhe courier Bao young adults"
        hints = _extract_appearance_hints(stage_direction, "courier Yuan")

        assert "courier Zhi Fu" in hints
        assert "Kou Zhao" in hints
        assert any("courier Bao" in hint for hint in hints)
...
class TestBackgroundGeneration:
    ...
    async def test_generate_with_heuristics(self):
        ...
        char_info = TemporaryCharacterInfo(
            character_name="courier Yuan",
            dialogues=["Nin courier to", "please Qian Shou"],
            stage_directions=[],
            scene_appearances=[3],
            first_appearance_scene=3,
            last_appearance_scene=3,
            dialogue_count=2,
            appearance_hints=["courier Zhi Fu"],
        )
    ...
    def test_heuristic_templates(self):
        ...
        known_types = ["courier Yuan", "doctor", "Hu Shi", "Jing Cha", "Fu Wu Yuan", "Si Ji"]
        ...
        for char_name in known_types:
            ...
            assert (
                result["personality"] != "Pu Tong、You Hao、Li Mao"
...
class TestAutoCreation:
    ...
    async def test_auto_create_workflow(
        ...
        script_content = {
            ...
            "stage_directions": [
                {"content": "wearing courier Zhi Fu young adults Zou Jin Lai", "scene_number": 3},
            ],
            "metadata": {
                "setting_location": "city Zhu Zhai Xiao Qu",
                "setting_time": "modern",
        ...
        assert char.character_name == "courier Yuan"
