...
def test_story_character_response_includes_virtual_ip_display_name(
    ...
    story = StoryFactory()
    virtual_ip = VirtualIPFactory(name="Lin Wan template")
    StoryCharacterFactory(
        story=story,
    ...
    assert character["virtual_ip_name"] == "Lin Wan template"
    assert character["name"] == "Lin Wan template"
    assert character["display_name"] == "Lin Wan template"
...
def test_episode_character_list_includes_virtual_ip_display_name(
    ...
    episode = Episode(story_id=story.id, episode_number=1, title="Di Yi Ji")
    virtual_ip = VirtualIP(user_id=user.id, name="courier Yuan template")
    ...
    assert item["virtual_ip_name"] == "courier Yuan template"
    assert item["name"] == "courier Yuan template"
    assert item["display_name"] == "courier Yuan template"
