...
def _create_story_episode_script(db_session, user: User, *, with_timeline: bool):
    ...
    episode = Episode(
        ...
        episode_number=6,
        title="truth Bi Jin",
        summary="key Zheng Ju Fu Chu Shui Mian。",
    )
    script = Script(
        episode=episode,
        title="Di6Ji script",
        content="Lin Shen：Bu Shi Ni Xiang Na Yang。",
                ...
                "beats": [
                    {
                        "start_ms": 78800,
                        "end_ms": 80780,
                        "dialogue_excerpt": "Bu Shi Ni Xiang Na Yang。",
                    }
                ],
            }
        }
...
def test_workbench_summary_ignores_stale_audio_timeline_metadata(
    ...
    """Jiu episode.extra_metadata.audio_timeline Bu Zai Rang timeline_ready for Zhen。"""
    user = _admin_user(db_session)
