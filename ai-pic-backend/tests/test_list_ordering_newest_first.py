...
def test_story_list_returns_newest_first(client, db_session):
 ...
 older = Story(title="Jiu Gu Shi", genre="drama", user_id=user.id)
 newer = Story(title="Xin Gu Shi", genre="drama", user_id=user.id)
...
def test_virtual_ip_list_returns_newest_first(client, db_session):
 user, _, _ = _bootstrap_episode(db_session)
 older = VirtualIP(name="JiuIP-Pai Xu", user_id=user.id)
 newer = VirtualIP(name="newIP-Pai Xu", user_id=user.id)
 db_session.add(older)
 db_session.commit()
 db_session.add(newer)
 db_session.commit()
