...
def test_story_list_returns_newest_first(client, db_session):
    ...
    older = Story(title="Jiu story", genre="drama", user_id=user.id)
    newer = Story(title="Xin story", genre="drama", user_id=user.id)
...
def test_virtual_ip_list_returns_newest_first(client, db_session):
    ...
    newer = VirtualIP(name="XinIP-Pai Xu", user_id=user.id)
    db_session.add(older)
    db_session.commit()
    db_session.add(newer)
    db_session.commit()
