...
async def test_full_image_generation_workflow(skip_if_no_openai):
 """test complete De image generate work Liu Cheng"""
 ...
 # call complete De image generate method
 ...
 if result:
 print(f" Fan Hui Zi Duan: {list(result.keys())}")
 if "local_file_path" in result:
 local_path = result["local_file_path"]
 print(f" Ben Di Wen Jian: {local_path}")
 print(f" Wen Jian Cun Zai: {os.path.exists(local_path) if local_path else False}")
